...
/** Stress testing suite for `reachableByFlows` queries under extreme conditions.
  ...
  *
  * These tests are designed to push the system to its limits while ensuring the consistency fixes remain effective
  * under stress.
  */
  ...
  "reachableByFlows stress tests" should {
    ...
    "handle deep call chains" in {
      ...
      depths.foreach { depth =>
        ...
        println(
          s"  Depth $depth: ${successfulResults.size}/$iterations successful, ${uniqueResults.size} unique results"
        )
