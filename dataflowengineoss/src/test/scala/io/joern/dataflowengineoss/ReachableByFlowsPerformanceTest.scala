...
class ReachableByFlowsPerformanceTest extends AnyWordSpec with Matchers {
  ...
  private def measurePerformance[T](testName: String)(operation: => T): (T, PerformanceMetrics) = {
    ...
    println(
      s"$testName: ${metrics.executionTimeMs}ms, ${metrics.memoryUsedMB}MB, ${metrics.resultCount} results, ${metrics.gcCount} GCs"
    )
  ...
  private def createScalabilityTestData(size: Int): Vector[String] = {
    ...
    combinations.toVector
      ...
      .toSet
      .toVector
      .sorted // Deterministic deduplication
  }
  ...
  "reachableByFlows performance tests" should {
    ...
    "compare sequential vs parallel-like execution performance" in {
      ...
      analyzePerformanceComparison("Sequential", sequentialMetrics.toVector, "Parallel-like", parallelMetrics.toVector)
    }
  ...
  private def analyzePerformanceComparison(
    name1: String,
    metrics1: Vector[PerformanceMetrics],
    name2: String,
    metrics2: Vector[PerformanceMetrics]
  ): Unit = {
