...
object CompilerAPI {
  ...
  // TODO: We need to calculate this value dynamically keeping 100 as minimum value
      ...
      .map { fileset =>
        ...
        defaultContentRootJarPaths.foreach { path =>
          if (!path.isResource) {
            ...
            if (f.exists()) {
              configCopy.add(CLIConfigurationKeys.CONTENT_ROOTS, new JvmClasspathRoot(f))
            ...
            if (resourceStream != null) {
              ...
              configCopy.add(CLIConfigurationKeys.CONTENT_ROOTS, new JvmClasspathRoot(tempFile))
        ...
        javaSourceRoots.foreach { source =>
          ...
          configCopy.add(CLIConfigurationKeys.CONTENT_ROOTS, new JavaSourceRoot(f, ""))
        }
        fileset.foreach { p =>
          configCopy.add(CLIConfigurationKeys.CONTENT_ROOTS, new KotlinSourceRoot(p, true, null))
        }
